...
class Ros2Serial(Node):
    ...
    def __init__(self, node_name="ros2serial_node"):
        ...
        self.declare_parameter('serial_port', '/dev/ttyUSB2') # Substitua pelo nome da porta correta - '/dev/ttyACM0'  ou '/dev/ttyUSB0'
    ...
    def robotState(self, valor):
        ...
        if valor[0] < -1300 and valor[2] < - 12000:
            robot.fallen_forward = True
        elif valor[1] < 7000 and valor[0] > 4000 and valor[2] > 0:
                robot.fallen_backwards = True
        elif valor[0] < -12000 and valor[1] > 0 and valor[2] > 0 or valor[0] > 12000 and valor[1] < 4000 and valor[2] < -3000:
    ...
    def imuSensor(self, valor):
        msg = Int32MultiArray()
        msg.data = [int(valor[0]), int(valor[1]), int(valor[2]), int(valor[3]), int(valor[4]), int(valor[5])]
                ...
                if self.ser is not None and self.ser.is_open and rclpy.ok():
                    try:
                        ...
                        if 'encoders' in data and rclpy.ok():
                            self.get_logger().info("CHEGOU AQUI")
                            try:
                                valor = data['encoders']
